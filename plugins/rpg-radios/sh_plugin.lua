...
if (CLIENT) then
	...
	surface.CreateFont("nutRadioFont", {
		...
		size = math.max(ScreenScale(7), 17*1.3),
		weight = 100
	})
	...
	netstream.Hook("radioAdjust", function(client, freq, id)
		...
		if (inv) then
			...
			if (id) then
				...
				item = inv:getFirstItemOfType("radio")
			end
...
nut.chat.register("radio", {
	...
	onChatAdd = function(speaker, text, anonymous)
		...
		chat.AddText(radCol,"[RADIO] ", radCol, speako, radCol, ": \""..finalMsg.."\"")
	end,
	prefix = {"/r", "/radio"},
})
